...
anchor = """function getConnector(request) {
...
"""
if anchor not in s:
    raise SystemExit('getConnector anchor missing')
...
old_cfg = """    connectTimeoutMs: clampInteger(env.CONNECT_TIMEOUT_MS, DEFAULT_CONNECT_TIMEOUT_MS, 500, 15000),
...
"""
new_cfg = """    connectTimeoutMs: clampInteger(env.CONNECT_TIMEOUT_MS, DEFAULT_CONNECT_TIMEOUT_MS, 500, 15000),
    idleTimeoutMs: clampInteger(env.IDLE_TIMEOUT_MS, DEFAULT_IDLE_TIMEOUT_MS, 10000, 600000),
...
old_health = "    if (url.pathname === '/health') return textResponse(users.length ? 'ok' : 'misconfigured', users.length ? 200 : 503);\n"
new_health = """    if (url.pathname === '/health') {
